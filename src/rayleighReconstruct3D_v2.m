function [clean, obj, times, mses] = rayleighReconstruct3D_v2(varargin)
...
%     end
    ...
    H(t+1) = sum(sum(sum(0.5*ysq./(Ax+1e-6)+log(Ax+1e-6))));
...
%     eu = exp(u);
    %
    obj(t+1) = alpha*TVnorm3d(x) + H(t+1);% + mu1*norm(Ax(:)-eu(:)-du(:))^2 + mu2*norm(x(:)-v(:)-dv(:))^2;
