/* 
 * =============================================================
 * arrayPrssIterate.cpp
 * Iterate within denoising algorithm loop (implemented in C++)
 * =============================================================
 */
#include <math.h>
#include "mex.h"
...
void arrayPrssIterate(double *x_o, double *x, double *y, double *x_n, int dim[3], double alpha)
{
   ...
   for (int k = 0; k < dim[2]; k++)            //sweeping planes
       for (int i = 0; i < dim[0]; i++)        // sweeping lines
           for (int j = 0; j < dim[1]; j++)    //sweeping column
           {
               ...
               E = 0.5*pow(y[c3(i,j,k,dim)],2)*exp(-x[c3(i,j,k,dim)]) + x[c3(i,j,k,dim)] + alpha*(1/g[c3(i,j,k,dim)])*(dx2+dy2+dz2); 
               nablaE = -0.5*pow(y[c3(i,j,k,dim)],2)*exp(-x[c3(i,j,k,dim)]) + 1 + 2*alpha*Nv(i,j,k,dim,g)*(x[c3(i,j,k,dim)] - meanX(i,j,k,dim,g,x));         
               nabla2E = 0.5*pow(y[c3(i,j,k,dim)],2)*exp(-x[c3(i,j,k,dim)]) + 2*alpha*Nv(i,j,k,dim,g);
               
               x_n[c3(i,j,k,dim)] = x[c3(i,j,k,dim)] - (nablaE/nabla2E);  
            }
    delete [] g;
}
...
// ----------------------------------------------------    
  ...
  x_o   = mxGetPr(prhs[0]); // Create a pointer to the input matrix x.
  x     = mxGetPr(prhs[1]); // Create a pointer to the input matrix x.
  y     = mxGetPr(prhs[2]); // Create a pointer to the input matrix y.
  ...
  arrayPrssIterate(x_o,x,y,x_n,dim,alpha); // Call the Cpp subroutine.
}